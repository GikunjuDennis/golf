	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'signup':
				...
				if(isTheseParametersAvailable(array('first_name','last_name','email','password'))){
					...
					$stmt = $pdo->prepare("SELECT user_id FROM users_102727 WHERE email = :email ");
					...
					if($stmt->fetchColumn() > 0){
					    ...
					    try {
					        ...
					        $sql = $pdo->prepare("INSERT INTO users_102727 (first_name,last_name,email,password) VALUES (? ,? ,?)");
                            $sql->execute(array($first_name,$last_name, $email, $password));
                			...
                			$stmt = $pdo->prepare("SELECT user_id, first_name,last_name,email FROM users_102727 WHERE email = :email AND password = :password ");
					$stmt->bindParam(':email', $email, PDO::PARAM_STR);
